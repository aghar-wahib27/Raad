...
import Vital as vit
from Assets.HandleFonts import FONT,getRequiredFont
from .HandleAssetsExtraction import UIS
from .clay_pythonized import *



ent.SYSTEMS['anim'].FadeScreen = ent.AnimF.ANIMS['AO'](ap=0,img=UIS['fade'],type='FD',display=vit.scr,position=(0,0),fade='',resize=vit.scr.get_size())

class ui(ent.concius,ent.EE,Node):
    def __init__(self,_name:str):
        self.de = ent.DT_SYS.MapDataEntity(_name)
        print(self.de,'printing self.de')
        self.name=_name
        self.positions=[]
        self.components=[]
        ent.concius.__init__(
            self,
            actions={'running':0},
            counters={'transion':20}
            )

        self.bgs = ent.Background(
    ...
    def behavior(self):
        for child in self.all_nodes:
            pygame.draw.rect(vit.scr,(child.size['w']%155,child.size['h']%155,50),child.noderect)

        for comp in self.components:
            comp.behavior()
            # pygame.draw.rect(vit.scr,(200,100,50),comp.rect)
...
@vit.singleton
class UIs(ent.container):
    def __init__(self):
        ent.container.__init__(self)
...
class ui_component(ent.concius):
...
def Functionailty():
    ...
    def Fade(dta):
            ent.SYSTEMS['anim'].StartFade(changes=dta['data'][0],new_ui=dta['data'][1],new_level=dta['data'][2])
...
class button(ui_component,Node):
    ...
    def behavior(self):
        # pygame.draw.rect(vit.scr,(self.size['w'],self.size['h'],50),self.children[0].noderect)
...
ent.SYSTEMS['ui'].general_components={'button':button,'representer':representer}
