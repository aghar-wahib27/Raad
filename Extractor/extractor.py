...
# consider using  os.path.abspath(".")
...
def getFileNameFromPath(path:str = None):
    if path and path.count('.')<2:
            name_with_extension = path.split(SEP)[-1].split('.')[0]
            return name_with_extension
    else :
        raise ValueError('path is none or contain alot of dots')
